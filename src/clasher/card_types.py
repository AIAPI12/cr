...
_CARDS_L11_PATH = os.path.join(os.path.dirname(__file__), '..', '..', 'ai', 'cards_l11.json')
_CARDS_L11 = {}
if os.path.exists(_CARDS_L11_PATH):
    try:
        with open(_CARDS_L11_PATH) as _f:
            _CARDS_L11 = _json.load(_f)
    except Exception:
        pass
L11_MULT = 1.10 ** 10

# Map internal gamedata names to display names used in L11 overrides
_L11_NAME_MAP = {
    'Assassin': 'Bandit',          # Bandit
    'Rascals': 'Rascal',           # Rascal
    'HeroIceGolem': 'IceGolem',    # IceGolem (champion variant)
    'IceGolemite': 'IceGolem',     # IceGolem (regular)
    'IceSpirits': 'IceSpirit',     # Ice Spirit
    'Ghost': 'RoyalGhost',         # Royal Ghost
    'Wallbreakers': 'WallBreakers',# Wall Breakers
}
...
class CardStatsCompat:
    ...
    def __init__(self, card_def: CardDefinition):
        ...
        # Apply L11 overrides: store as level-1 base values
        l11_name = _L11_NAME_MAP.get(self.name, self.name)
        l11 = _CARDS_L11.get(l11_name, {})
        if l11:
            if 'hp' in l11 and l11['hp']:
                self.hitpoints = int(l11['hp'] / L11_MULT)
            if 'dmg' in l11 and l11['dmg']:
                self.damage = int(l11['dmg'] / L11_MULT)
    ...
    def get_scaled_stat(self, stat_value: Optional[int], stat_name: str = 'hp',
                        level: int = None) -> Optional[int]:
        """Scale stat by level using uniform ~1.10 per level (CR standard)."""
        if stat_value is None:
            return None
        lvl = level if level is not None else self.level
        multiplier = 1.10 ** max(0, (lvl - 1))
        return int(stat_value * multiplier)

    @property
    def scaled_hitpoints(self):
        return self.get_scaled_stat(self.hitpoints, 'hp')

    @property
    def scaled_damage(self):
        return self.get_scaled_stat(self.damage if self.damage is not None else 0, 'dmg')

    @property
    def scaled_damage_special(self):
        return self.get_scaled_stat(self.damage_special if self.damage_special is not None else 0, 'dmg')
