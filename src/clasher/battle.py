...
# Tower stats at tournament standard (level 11) — loaded from gamedata when available
TOWER_TROOP_STATS = {
    'PrincessTower': {'hitpoints': 2534, 'damage': 90, 'range': 7.5, 'hit_speed_ms': 800, 'sight_range': 7.5},
    'Cannoneer': {'hitpoints': 2800, 'damage': 230, 'range': 7.0, 'hit_speed_ms': 1000, 'sight_range': 7.0, 'splash': True},
    'DaggerDuchess': {'hitpoints': 2200, 'damage': 40, 'range': 6.5, 'hit_speed_ms': 300, 'sight_range': 6.5},
}

def pick_tower_troops():
    return 'PrincessTower', 'PrincessTower'
...
class BattleState:
    ...
    tower_troop: List[str] = field(default_factory=lambda: ['PrincessTower', 'PrincessTower'])
    _starting_total_tower_hp: Dict[int, float] = field(default_factory=dict, init=False)
    _sudden_death_crowns: List[int] = field(default_factory=lambda: [0, 0])
    ...
    def _get_tower_stats(self, pid: int) -> dict:
        tt = self.tower_troop[pid] if pid < len(self.tower_troop) else 'PrincessTower'
        return TOWER_TROOP_STATS.get(tt, TOWER_TROOP_STATS['PrincessTower'])

    def _create_tower_for_player(self, pid: int, is_left: bool) -> None:
        ts = self._get_tower_stats(pid)
        if pid == 0:
            pos = Position(self.arena.BLUE_LEFT_TOWER.x, self.arena.BLUE_LEFT_TOWER.y) if is_left else Position(self.arena.BLUE_RIGHT_TOWER.x, self.arena.BLUE_RIGHT_TOWER.y)
        else:
            pos = Position(self.arena.RED_LEFT_TOWER.x, self.arena.RED_LEFT_TOWER.y) if is_left else Position(self.arena.RED_RIGHT_TOWER.x, self.arena.RED_RIGHT_TOWER.y)
        tstats = building_from_values(
            name="Tower", hitpoints=ts['hitpoints'], damage=ts['damage'],
            range_tiles=ts['range'], sight_range_tiles=ts.get('sight_range', ts['range']),
            hit_speed_ms=ts['hit_speed_ms'], deploy_time_ms=800,
            collision_radius_tiles=1.0, lifetime_ms=None, elixir=0, rarity="Common",
            projectile_speed=600, projectile_damage=ts['damage'],
            target_type="TID_TARGETS_AIR_AND_GROUND", raw_overrides={"level": 11},
        )
        self._spawn_entity(Building, pos, pid, tstats)

    def _create_towers(self) -> None:
        """Create tower entities for both players"""
        for pid in range(2):
            ts = self._get_tower_stats(pid)
            self.players[pid].left_tower_hp = ts['hitpoints']
            self.players[pid].right_tower_hp = ts['hitpoints']
            self._create_tower_for_player(pid, True)
            self._create_tower_for_player(pid, False)

        king_stats = building_from_values(
            name="KingTower", hitpoints=4824, damage=109,
            range_tiles=7.0, sight_range_tiles=7.0,
            hit_speed_ms=1000, deploy_time_ms=1000,
            collision_radius_tiles=1.0, lifetime_ms=None, elixir=0, rarity="Common",
            projectile_speed=600, projectile_damage=109,
            target_type="TID_TARGETS_AIR_AND_GROUND", raw_overrides={"level": 1},
        )
        for pid, kpos in [(0, self.arena.BLUE_KING_TOWER), (1, self.arena.RED_KING_TOWER)]:
            pos = Position(kpos.x, kpos.y)
            self._spawn_entity(Building, pos, pid, king_stats)
